...
#define CTL_UNLOCK_ALM_MASK (1 << 7) 
...
__declspec(align(256)) intptr_t volatile sg_thp_mask = 0;
__declspec(align(256)) intptr_t volatile sg_spinlock = 0;
...
repeat:
   ...
   assert (isr == FX1S_OK);
   if (++ try_ > 120) {
     MessageBoxA (NULL, "始终读不出数据, 请检查接线情况/PLC以及相关设备电源情况在启动程序", "fx1s_timer", MB_ICONERROR);
     CloseHandle (sg_ctl.comport); /* XXX:so simple... **/
     exit (-1);
   }
...
static
int __stdcall thp_monitor (void *param) {
   ...
   __try {
     do {
       int64_t elsp;
       if (sg_thp_mask & CTL_UNLOCK_ALM_MASK) {
         sg_thp_mask &= ~CTL_UNLOCK_ALM_MASK;
         obt = readOYB (FX1S_VERSION_14MR, 4);
         obt &= ~(1 << (4 & 7));
         setOYB (FX1S_VERSION_14MR, 4, obt);
       }
       ...
       if (sg_thp_mask & CTL_FORCE_STOP_MASK) {
         ...
         if (sg_listset.res_line != -1) {
           ...
           sg_listset.res_line = -1;
           ListView_EnsureVisible (window_main_listhook, lines, FALSE);
       ...
       if (elsp <= iq_curr) {
         ...
         if (sg_ctl.pd_poll == sg_ctl.xe_poll)
           continue;
         /* deal current C register **/
         cc_timing = readPULSE ();

         if (sg_ctl.pd_poll->setv <= cc_timing) { /* current pulse-count >= setting ?? */
           /* Trigger work **/
           ...
           ListView_EnsureVisible (window_main_listhook, lines + 1, FALSE);
...
INT_PTR CALLBACK hook_listhook (HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
  if (message == WM_VSCROLL && window == window_main_listhook)
  {
    SendMessage (window_main_editattach, WM_KILLFOCUS, 0, 0);
...
INT_PTR CALLBACK dialog_proc (HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
  ...
  case WM_INITDIALOG:
    ...
    callback_hook_listhook = (void *)SetWindowLong (window_main_listhook, GWL_WNDPROC, (LONG)hook_listhook);
  ...
  case WM_COMMAND:
    if (LOWORD(wparam) == IDEM_COMSET) {
      ...
      CloseHandle (sg_ctl.timingth);
             /* cancel alert PLC **/
      if (sg_ctl.comport != INVALID_SET_FILE_POINTER)
      {
       obt = readOYB (FX1S_VERSION_14MR, 4);
       obt &= ~(1 << (4 & 7));
	  ...
	  EndDialog (window, 0);
      break;
	} else if (LOWORD(wparam) == IDCM_BUTTON_LOCKALM) {
      sg_thp_mask |= CTL_UNLOCK_ALM_MASK;
      break;
    }
       ...
       */
       ...
       obt &= ~(1 << (4 & 7));
       setOYB (FX1S_VERSION_14MR, 4, obt);
       ListView_EnsureVisible (window_main_listhook, 0, FALSE);
       /* create thread ***/
       sg_ctl.timingth = (void *)_beginthreadex (NULL, 0,  thp_monitor, NULL, 0, NULL);
       assert (sg_ctl.timingth != (void *)-1);
...
{
  if (licence_check () == FALSE)
  {
    MessageBoxA (NULL, "想要取得使用权请联系软件开发者\nsosicklove@yahoo.com", "moecmks", MB_OK);
    return 0;
  }
  DialogBox (instance, MAKEINTRESOURCE(IDD_DIALOG_MAIN), NULL, dialog_proc);
  return 0;
}
